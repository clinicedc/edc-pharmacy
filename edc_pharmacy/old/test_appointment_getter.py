...
class TestAppointmentGetter(TestCase):
    ...
    def test_schedule_subject_next(self):
        ...
        self.assertNotEqual(
            dispense_timepoint.appt_datetime, dispense_next.appt_datetime
        )
        self.assertEqual(
            dispense_timepoint.next().appt_datetime, dispense_next.appt_datetime
        )
    ...
    def test_schedule_subject_previous(self):
        ...
        self.assertEqual(
            dispense_current.previous().appt_datetime, dispense_timepoint.appt_datetime
        )
    ...
    def test_dispense_schedule_current(self):
        dispense_schedule = (
            DispenseSchedule.objects.filter(
                subject_identifier=self.randomized_subject.subject_identifier
            )
            .order_by("created")
            .first()
        ...
        self.assertEqual(dispense_schedule.start_datetime.date, date(2017, 8, 24))
