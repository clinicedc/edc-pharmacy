...
class TestPeriod(TestCase):
    ...
    def test_of_days_weekend_allowed(self):
        period = Period(start_datetime=datetime(
            2017, 9, 25), unit=DAYS, duration=2)
        self.assertEqual(period.start_datetime.date(), date(2017, 9, 25))
    ...
    def test_of_weeks(self):
        ...
        self.assertEqual(period.start_datetime.date(),
                         datetime(2017, 8, 24).date())
        self.assertEqual(period.end_datetime.date(),
                         datetime(2017, 8, 30).date())

    @tag('period_holidays')
    def test_period_holidays(self):
        period = Period(start_datetime=datetime(
            2017, 9, 29), unit=DAYS, duration=2)
        day1, day2 = period.workdays
#         start_datetime = arrow.Arrow.fromdatetime(
#             datetime(2017, 8, 24)).datetime
        self.assertEqual(day1.date(),
                         datetime(2017, 9, 29).date())
        self.assertEqual(day2.date(),
                         datetime(2017, 10, 1).date())
