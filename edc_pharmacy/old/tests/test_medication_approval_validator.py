...
from ..medications.prescription_approval_validator import (
...
class TestMedicationApprovalValidator(TestCase):
    ...
    def test_validator(self):
        ...
        prescriptions = Prescription.objects.filter(dispense_appointment=appointment)
        PrescriptionCreator(dispense_appointment=appointment, options=self.options)
        self.assertEqual(
            Prescription.objects.filter(dispense_appointment=appointment).count(), 2
        )
